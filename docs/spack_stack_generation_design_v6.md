...
| **Release** | One concretized, built, verified copy of the stack with a unique tag (e.g. `2026.06`). Saved under `releases/<tag>/`. The `current` symlink points at the active release. |
| **Foundation cache** | The build-cache lane that holds Core builds. Keyed by OS/glibc + baseline target (e.g. `foundation/rhel8/x86_64_v3`). Read by every lane on a matching system. |
| **Science cache** | The build-cache lane that holds science (serial/MPI/GPU) builds. Keyed by OS/glibc; compiler/MPI/target appear in the directory label for human readability, not as reuse boundaries. |
...
compilers_external:                             # O - non-Cray site or system compilers as externals
  - name: aocc
    version: "4.2.0"
...
mpi:                                            # O - non-Cray MPI implementations available on the system
  - name: openmpi
...
node_types:                                     # R - one entry per node class on the system
  ...
  gpu_compute_mi250x:
    ...
    gpu:                                        # R within gpu: present when class has a GPU
      ...
      arch_target: gfx90a                       # R - gfx90a | gfx942 | sm_80 | sm_90 | ...
...
The render step treats absent optional keys as defaults. A profile with no
`gpu:` block produces a workspace with no GPU lanes; a profile with no
`vendor_cray:` block produces a workspace with no Cray scope includes.
...
targets:                                        # R - target policy by class
  ...
  science_default: zen3                         # R - optimized target name; render resolves against profile.cpu.preferred
...
lanes:                                          # R - the matrix the render step expands
  - name: gcc-core                              # R - identifier; used in env paths and module names
    ...
    lane: core                                  # R - core | serial | mpi-<provider> | gpu-<provider>
    kind: core                                  # R - core | serial | mpi | gpu
...
versions:                                       # O - multi-version policy per package
  hdf5:        ["1.14.5", "1.14.4", "1.12.3"]   # O - science-lane multi-version
  netcdf-c:    ["4.9.2", "4.9.0"]
  foundation_pinned:                            # O - libs the common scope must require: single-version
    zlib:  "1.3.1"
    xz:    "5.4.6"
    zstd:  "1.5.6"

buildcache:                                     # O - build-cache lane policy
  foundation_lane: "foundation/{os}/{baseline_target}"
  science_lane:    "science/{system}/{compiler}-{lane}/{target}"
...
| `targets.science_default` | `configs/target/<target>/packages.yaml` rendered into serial/MPI/GPU lanes |
...
| `versions.<package>` | multi-version specs in the science lane |
| `versions.foundation_pinned.<lib>` | `require:` lines in the common scope |
...
specs:                                          # R - root specs by lane-kind constraint
  any:                                          # specs that apply to every kind in `kinds`
    - hdf5+fortran
    - netcdf-c
    - netcdf-fortran
...
list. The toolchain decoration (`%cce_craympich` etc.) is applied by the
render step from the lane's compiler-and-MPI pairing, not from the package
set; sets stay compiler-agnostic so the same set is usable across lanes.
...
| `modules.yaml` | Spack-generated module behavior | The full front-door stack module design unless delegated to Spack |
...
## Detailed Scenario: Regular Linux With Site MPI
...
function render(profile_path, stack_path, package_sets_dir, templates_dir,
    ...
    require(stack_lanes_compatible_with_profile(stack, profile),
            "stack lane requires capability profile does not declare")

    # ── Context ───────────────────────────────────────────────────────────
    ctx = build_render_context(profile, stack, sets, release_vars)
    ...
    mkdir_clean(workspace)
    pending = workspace + ".rendering"   # write to side path, rename atomically

    # ── Config scopes ─────────────────────────────────────────────────────
    for scope_name in stack.required_scopes(profile):
    ...
    write_yaml(pending / "release-manifest.yaml",
               build_manifest(ctx, pending))

    # ── Commit ──────────────────────────────────────────────────────────
    atomic_rename(pending, workspace)
    return workspace
...
The functions called by `render` (`scopes_for_lane`, `toolchain_for_lane`,
`runtime_modules_for_lane`, `view_root`, `expand_package_set`,
`stack_lanes_compatible_with_profile`, `build_manifest`) are pure
transformations of the frozen context. None of them touches the host.
...
- A lane references a compiler the profile does not declare.
- A GPU lane is requested but the profile has no `gpu:` block.
- A Cray lane is requested but the profile has no `vendor_cray:` block.
...
function probe_node(args) -> node_fragment:
    ...
    n.build_stage = probe_writable_fast_paths()
    return n
...
function probe_gpu() -> gpu_block_or_None:
    if has_nvidia_smi():
        ...
        arch    = detect_compute_capability()          # sm_80, sm_90, ...
        return {
            vendor: "nvidia",
...
- Apply the same playbook to Cray and non-Cray systems by varying per-host
...
| `provision` | Create shared directories (`install_tree`, `source_cache`, `build_stage`, `buildcache`, release dirs); set permissions; place the rendered workspace at the release dir. |
| `concretize-fetch` | On the login node, run `spack -e <env> concretize` and `fetch -D` per lane; save `spack.lock`. |
| `install` | Submit per-lane scheduler jobs (Slurm/PBS) for `spack install -j N`. Track outcomes; collect logs. |
| `verify` | Run `spack verify libraries`, `spack verify manifest -a`, and the site smoke tests per lane. |
| `publish` | Regenerate views and modules; push the build cache; write `release-manifest.yaml`. |
| `promote` | Gated symlink swap (`ln -sfn`). Refuses to delete a previous release tree if `current` points at it. |
...
roles:
  ...
  - role: provision
    tasks:
      - ensure_dirs: [install_tree, source_cache, build_stage, buildcache, release_dir]
      - rsync:
          src:  "{{ workspace }}/"
          dest: "{{ release_dir }}/"
  ...
  - role: verify
    tasks:
      - for lane:
          ...
          run: {{ site_smoke_test_command }}
      - assert: all lanes passed
...
buildcache_mirror:        file:///shared/stack/buildcache/science/example-cray
foundation_mirror:        file:///shared/stack/buildcache/foundation/rhel8/x86_64_v3
...
The same config-scope grammar should work for Cray and regular Linux systems.
...
### Per-Lane Core, Not Shared Core

**The committed model is per-lane Core.** Every compiler on a system owns its
...
This is the explicit, intentional cost of the per-lane model: build tools and
stable-ABI libraries are duplicated across compilers within a system. The
benefits make the duplication worthwhile:
...
- **No `include_concrete` plumbing.** A shared-Core scheme requires deriving
  ...
  `reuse: from:`. Per-lane Core needs none of that — every lane is a normal
...
- **No ABI guessing.** "Is GCC-built zlib safe for a CCE consumer?" is a
  ...
  at all for ABI-coupled ones. Per-lane Core does not need to make that call;
...
fallback the per-lane model depends on. Until the evidence appears, the
per-lane default is the model, and the rest of the design does not assume
shared-Core extraction has happened.
...
| Cray, full Option A NVHPC exception lane added | 2 + 1 (NVHPC narrow) | core, serial, mpi, gpu, gpu-exception | **9** | the 8 above plus `nvhpc/gpu-craympich-sm90` |
...
- GPU architecture target is a build axis, for example `sm_90`, `gfx90a`, or `gfx942`.
...
Add this to the Committed Decisions row on host-compiler policy: on Cray
PE specifically, the "GNU + GPU toolkit" pattern is realized by
**PrgEnv-gnu + standalone toolkit module** (Option B), not by PrgEnv-amd
or PrgEnv-nvidia. The exception lanes are named, scoped, and renderable
...
**Build-stage placement.** The build stage should live on the fastest
writable path the inspector found (typically `/local_scratch/$user/...` on
a compute node, `$TMPDIR/...` inside an allocation). Keep `source_cache`
and `install_tree` on shared storage so every node sees the same artifact
set, but never put `build_stage` on shared storage if a local fast path
exists — Make/Ninja are I/O-heavy and shared filesystems multiply the
write latency through every dependent.
...
- Prepend the compiler's Core module root to `MODULEPATH` (per-lane Core, so
...
- **Site-external lane → runtime modules required.** A lane built on
  module-provided externals — Cray PE compilers and cray-mpich, or a Linux
  lane on a site compiler and site MPI — carries a runtime dependency on
  those same modules. The site compiler's runtime libraries and the site
  MPI's launcher and libraries live behind the site's modules; without them
  loaded, user-fresh compiles fail to find the compiler driver and MPI
  programs fail to find `mpirun`. RPATH covers the stack's own binaries'
  linkage but not the user's fresh compile path or the MPI launcher's
  search.
...
against. The same shape applies to a Linux AOCC + site-OpenMPI lane: the
front-door module loads `aocc/4.2.0` and the site MPI module, and the rest
of the lane's user surface (PATH, CMAKE_PREFIX_PATH, etc.) is the stack's
view paths layered on top.
...
conflict CSE/CCE/gpu-craympich
conflict CSE/GCC/serial
conflict CSE/GCC/mpi-craympich
conflict CSE/GCC/gpu-craympich
conflict CSE/ROCmCC/core
conflict CSE/ROCmCC/gpu-craympich
...
setenv STACK_VIEW      "/shared/stack/current/example-cray/cse/views/cce/mpi-craympich"

# ── MODULEPATH: compose Core + lane (per-lane Core, same compiler) ───────
prepend-path MODULEPATH "/shared/stack/current/example-cray/cse/modules/cce/core"
prepend-path MODULEPATH "/shared/stack/current/example-cray/cse/modules/cce/mpi-craympich"
...
# Root of the projected view entry for this package.
set root "/shared/stack/current/example-cray/cse/views/cce/mpi-craympich/hdf5/1.14.5"
...
different `+cuda_arch` or different `^mpi` choices. If you find yourself
...
different release directories. The `current` symlink swap is what makes a
release "live"; users always go through `/shared/stack/current/...` and
never reference a release tag directly. A release-tagged path is still
usable for archaeology — `find /shared/stack/releases/2026.05/...` works —
but the supported user surface is `/shared/stack/current/...`.
...
```text
buildcache/foundation/rhel8/x86_64_v3            # OS=rhel8 is the key; foundation is the label
...
every `science/example-cray/*` lane as read sources, regardless of which
compiler appears in the directory label, so any compatible binary is
reachable.
...
lanes:                                         # one entry per environment in the release
  ...
  - name: cce-mpi-craympich
    ...
    module_root: "/shared/stack/releases/2026.06/example-cray/cse/modules/cce/mpi-craympich"
    package_set: science-full
    provenance_summary:
      stack_built: 18
  ...
  - name: gcc-gpu-craympich-gfx90a              # Cray PE + GPU lane assembly Option B:
    ...
    module_root: "/shared/stack/releases/2026.06/example-cray/cse/modules/gcc/gpu-craympich-gfx90a"
    package_set: science-gpu
    provenance_summary:
      stack_built: 12
...
buildcache:
  push_destinations:                           # mirrors this release was pushed to
    - name: foundation
      url:  "file:///shared/stack/buildcache/foundation/rhel8/x86_64_v3"
...
```bash
ln -sfn releases/2026.06 /shared/stack/current
```
...
| `spack concretize` fails with "no satisfying spec for compiler" | Profile declared the compiler but stack's `externals.compilers` policy did not honor it; or the toolchain pins a compiler the lane cannot reach | Cross-check `profile.compilers_external` vs `stack.externals.compilers`; check `toolchains.yaml` versions match the profile |
...
| GPU runtime fails with "CUDA driver version is insufficient for CUDA runtime version" | Toolkit version exceeds the GPU driver ceiling | `profile.gpu.toolkit_ceiling`; pin CUDA in `configs/gpu/nvidia-cuda/packages.yaml` at or under it |
| GPU build fails with "no kernel image available for execution" | `cuda_arch` / `amdgpu_target` does not match the profile's `gpu.arch_target` | `configs/gpu/<vendor>/packages.yaml` — set the correct arch target |
...
| Different OS lanes share one cache and hashes seem to overlap | Cache lane is not keyed by OS/glibc; SLES and RHEL builds collided | Re-key cache by OS/glibc — `buildcache/foundation/<os>/<baseline>/` |
| Two compilers' Core CMake collide in one view | One shared Core view across compilers — per-lane Core needs per-compiler view roots | View root template — split into `views/<compiler>/core/` |
...
PASS  every lane.compiler resolves in profile.compilers_external
...
```bash
$ cd /tmp/rendered/example-cray/cse/2026.06
...
Per lane, verify integrity, push the science cache, regenerate views and
modules:
...
Copy the workspace + lockfiles to the release directory and write the
manifest:
...
The two MPI lanes differ in their front-door modules: the site-MPI lane
loads `aocc/4.2.0` and `openmpi/4.1.6` at runtime (site-external lane,
needs the runtime modules); the Spack-built OpenMPI lane loads nothing
at runtime (fully self-contained, RPATH covers everything).

```bash
# A user picks the MPI flavor they want
$ module load CSE/AOCC/mpi-site            # site-tuned, runtime aocc+openmpi modules
$ module list | grep -E "aocc|openmpi|CSE"
1) aocc/4.2.0    2) openmpi/4.1.6    3) CSE/AOCC/mpi-site

# vs.
$ module swap CSE/AOCC/mpi-site CSE/AOCC/mpi-openmpi   # stack-owned, self-contained
$ module list | grep -E "aocc|openmpi|CSE"
1) CSE/AOCC/mpi-openmpi                       # no aocc/openmpi runtime modules needed
```
...
|---|---|---|
| Helper command names | `cluster-inspector` (the read-only system probe), `stack-render` (the render helper), `stack-validate` (validate-only mode), `stack-explain` (dry-run mode). Ansible playbook names follow `deploy-stack`. | A naming review happens before the helpers are user-installable; until then these names are committed. The design itself does not bind a name and the rules in §Render Step would survive a rename. |
...
| Build-cache keying | **OS/glibc only**. Compiler/MPI/target are directory labels for human readability, not reuse boundaries. See §Build-Cache Keying. | Never — the hash already enforces ABI correctness; per-compiler keying actively strands the foundation. |
...
| Naming on user-facing modules | `<front_door>/<compiler>/<lane>` — e.g., `CSE/CCE/mpi-craympich`. The system name is not in the user-facing module path; it shows up in internal release directories and in `STACK_RELEASE`. | Never. |
...
|---|---|---|
| First two systems to prove the design end-to-end | One Cray + one regular Linux. The Cray slot is whichever Cray-PE system has compute time available first; the Linux slot is a Penguin-class node with site OpenMPI. | When two specific candidate systems are named, lock them in §Example Cray Flow / §Example Regular Linux Flow. |
...
| Where `release-manifest.yaml` history lives | Committed under `releases/<tag>/release-manifest.yaml` in the source repo. CI/release records may *additionally* link to it. | Repo size becomes a practical issue. |
